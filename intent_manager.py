...
def intent_manager_fun(intent_dict_main, workflow_url, whatif_send_url, whatif_receive_url,
    ...
    columns = ['intent_id', 'intent_type', 'threat', 'host', 'action', 'duration', 'priority']
    ...
    if policy_dict['intent_type'] == 'mitigation':
        ...
        whatif_answer = get_intents.get_intent_fun(whatif_receive_url)[0]
    ...
    if to_proceed == True:
                        ...
                        for ind in df.index:
                            if df['host'][ind] == intent_host_arr[j] and df['threat'][ind] == intent_dict_main['threat'] and ind < len(df.index):
                                if policy_dict['priority'] >= df['priority'][ind]:
                                    ...
                                    policy_dict['duration'] = df['duration'][ind]
        ...
        for ind in df.index:
            for ip in policy_dict['host']:
                ...
                policy_send['duration'] = str(policy_dict['duration'])
                ...
                if policy_send['intent_type'] == df['intent_type'][ind] and policy_send['threat'] == df['threat'][ind] and \
                        ...
                        policy_send['duration'] == str(df['duration'][ind]):
